...
QueueHandle_t rcQueueHandle;
QueueHandle_t stepperQueueHandle;
...
void MX_FREERTOS_Init(void) {
  ...
  /* 创建遥控数据队列（NRFTask→ControlTask） */
  rcQueueHandle = xQueueCreate(1, sizeof(RC_Data_t));
  /* 创建步进电机命令队列（ControlTask→StepperTask） */
  stepperQueueHandle = xQueueCreate(1, sizeof(StepperCmd_t));
  ...
  // osThreadDef(emergencyTask, StartEmergencyTask, osPriorityRealtime, 0, 128);
  ...
  osThreadDef(nrfTask, StartNRFTask, osPriorityAboveNormal, 0, 128);
  nrfTaskHandle = osThreadCreate(osThread(nrfTask), NULL);

  /* definition and creation of oledTask */
  osThreadDef(oledTask, StartOLEDTask, osPriorityLow, 0, 128);
  oledTaskHandle = osThreadCreate(osThread(oledTask), NULL);

  /* definition and creation of stepperTask */
  osThreadDef(stepperTask, StartStepperTask, osPriorityNormal, 0, 128);
