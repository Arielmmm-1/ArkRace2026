...
//	/*显示初始界面*/
//	OLED_Clear();
//	OLED_ShowString(0, 0,  "     [GDELE]   ", OLED_8X16);
...
/* ========== 车端回传的 QR 解析结果 ========== */
static char QR_base[16]   = {0};
static char QR_core[16]   = {0};
static char QR_left[16]   = {0};
static char QR_middle[16] = {0};
static char QR_right[16]  = {0};

#define AD_DEAD			100

int8_t DataProcess(uint16_t ADValue);
uint8_t CalculateSuccessRatio(uint8_t SendFlag);
void     ParseQRData(char *str);
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (Flag)
        {
            ...
            {
                /* 首字节 'Q' 表示 QR 数据包 */
                if (NRF24L01_RxPacket[0] == 'Q')
                {
                    ParseQRData((char *)&NRF24L01_RxPacket[1]);
                }
            }
        }
        ...
        if (Mode == 0)
        {
            /* ----- 模仿 OLEDTask：显示五个 QR 字段 ----- */
            OLED_ShowString(0,  16, "B:", OLED_8X16);
            OLED_ShowString(16, 16, QR_base, OLED_8X16);
            OLED_ShowString(72, 16, "C:", OLED_8X16);
            OLED_ShowString(88, 16, QR_core, OLED_8X16);

            OLED_ShowString(0,  32, "L:", OLED_8X16);
            OLED_ShowString(16, 32, QR_left, OLED_8X16);
            OLED_ShowString(72, 32, "M:", OLED_8X16);
            OLED_ShowString(88, 32, QR_middle, OLED_8X16);

            OLED_ShowString(0,  48, "R:", OLED_8X16);
            OLED_ShowString(16, 48, QR_right, OLED_8X16);
        }
        /* Mode 1：可扩展其他显示，暂留空 */
